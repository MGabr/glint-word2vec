...
__all__ = ['NaiveGlintWord2Vec', 'NaiveGlintWord2VecModel', 'ServerSideGlintWord2Vec', 'ServerSideGlintWord2VecModel']
...
class ServerSideGlintWord2VecModel(JavaVectorTransformer, JavaSaveable, JavaLoader):
    """
    class for Word2Vec model
    .. versionadded:: 1.2.0
    """
    ...
    def load(cls, sc, path):
        ...
        """
        jmodel = sc._jvm.org.apache.spark.mllib.feature \
            .ServerSideGlintWord2VecModel.load(sc._jsc.sc(), path)
        model = sc._jvm.org.apache.spark.mllib.api.python.ServerSideGlintWord2VecModelWrapper(jmodel)
...
@ignore_unicode_prefix
class ServerSideGlintWord2Vec(object):
    """Word2Vec creates vector representation of words in a text corpus.
    The algorithm first constructs a vocabulary from the corpus
    and then learns vector representation of words in the vocabulary.
    The vector representation can be used as features in
    ...
    def fit(self, data):
        ...
        jmodel = callMLlibFunc("trainWord2VecModel", data, int(self.vectorSize),
                               ...
                               int(self.numIterations), self.seed,
                               int(self.minCount), int(self.windowSize))
        return ServerSideGlintWord2VecModel(jmodel)
